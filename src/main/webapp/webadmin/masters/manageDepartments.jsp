...
<% 
	...
	if(request.getParameter("id") != null && request.getParameter("action") != null && !request.getParameter("action").equals("")){
		...
		if(strType.equals("Active")){
			strUpdateQuery = "UPDATE CMS_DEPARTMENTS SET CMSDEPT_FLAG=1 WHERE CMSDEPT_ID="+request.getParameter("id")+"";
		...
		if(strType.equals("DeActive")){
			...
			<%}else{
				strUpdateQuery = "UPDATE CMS_DEPARTMENTS SET CMSDEPT_FLAG=0 WHERE CMSDEPT_ID="+request.getParameter("id")+"";
				dbBean.setQuery(strUpdateQuery);
				dbBean.go1();
			}	strMSG = "Update Successfully";
		}
...
<form action="manageDepartmentAction.jsp" method="post">
...
<input type="submit" name="AddDivision" value="Add Department">
</td>
</tr>
