...
response.sendRedirect("http://"+request.getServerName()+"/cris/webadmin/home.jsp"); 
}
%>
<script>
function chkForm(){
...
String query_right;
query_right = "SELECT DAU_NAME,DAU_EMAIL,DAU_ACCESS_LEVEL,DAU_STATUS,DAU_ID,DEPT_ID FROM CMS_ADMIN_USER";
	...
	if (getright.size()!=0){
		for (int i=0;i<getright.size();i++){%>
			...
			<td align="center"><%=i+1%></td>
				<td><%=dbBean.getcell(getright,0,i)%></td>
				
				<%if(Hname.equals("wr")){%>
				<td><%
				String dept = dbBean.getcell(getright,5,i);
				String depts[]=dept.split(",");
				for(int n=0;n<depts.length;n++)
				{
				Vector getdept = new Vector();
				String query_dept;
				query_dept = "SELECT CMSDEPT_NAME FROM CMS_DEPARTMENTS WHERE CMSDEPT_ID="+depts[n]+"";
				dbBean.setQuery(query_dept);
				getdept=(Vector)dbBean.go();
				if (getdept.size()!=0)
				out.print(dbBean.getcell(getdept,0,0)+"<br>");
				}
				%>
				</td>
				<%}%>
				
				<td align="center"><%=dbBean.getcell(getright,1,i)%></td>
