...
<p align="center"><span align="center" class="mgContentTitle">User Log Details</span></p>
<br>
<br><br> 
<%
	String strDate = "";
	String condition = "";
	String strTenderCount = "SELECT COUNT(*) FROM CMS_ADMIN_LOG WHERE CAL_CAU_LOGIN = '"+(String)session.getAttribute("DBAusername")+"'";
	String strTenderDetails ="SELECT CAL_CAU_LOGIN,CAL_TITLE,CAL_DESCRIPTION,CAL_SQLQUERY,DATE_FORMAT(CAL_CREATED_DATE,'%d-%m-%Y %H:%i'),CAL_ID,CAL_IPADDRESS FROM CMS_ADMIN_LOG WHERE CAL_CAU_LOGIN = '"+(String)session.getAttribute("DBAusername")+"' ORDER BY CAL_CREATED_DATE DESC ";
...
<table border="1" class="sample" width="100%">
		...
		<tr>
			...
			<td nowrap="nowrap" align="center"><%=rsGetNoticDetails[l][4]%></td>
			<td align="center"><%=rsGetNoticDetails[l][6]%></td>
			<td align="center"><%=rsGetNoticDetails[l][1]%></td>
			
			<td align="center"><a href="userLogDetails.jsp?id=<%=rsGetNoticDetails[l][5]%>&keepThis=true&TB_iframe=true&height=500&width=600" class="thickbox""><font color="black">View Details</font></a></td>
...
<table align="center">
	...
	<%if(TotalPages > 1){%>
			<%if(Start > 1){%>
			<td><a href="logDetails.jsp?Start=<%=encrypt("1")%>&byDate=<%=strDate%>&userId=<%=strUserId%>"></P><font color="black"><b>First</b></font></a>&nbsp;&nbsp;</td>
			<td><a href="logDetails.jsp?Start=<%=encrypt(Integer.toString(Start-1))%>&byDate=<%=strDate%>&userId=<%=strUserId%>"></P><font color="black"><b>Previous</b></font></a>&nbsp;&nbsp;</td>
			<%}if(TotalPages > 1 && Start < TotalPages){%>
			<td>&nbsp;&nbsp;<a href="logDetails.jsp?Start=<%=encrypt(Integer.toString(Start+1))%>&byDate=<%=strDate%>&userId=<%=strUserId%>"><font color="black"><b>Next</b></font></a></td>
			<td>&nbsp;&nbsp;<a href="logDetails.jsp?Start=<%=encrypt(Integer.toString(TotalPages))%>&byDate=<%=strDate%>&userId=<%=strUserId%>"><font color="black"><b>Last</b></font></a></td>
			<%}%>
