...
<%
String strGetUserQuery = "SELECT DISTINCT USER_NAME FROM cms_login_failed_details ORDER BY USER_NAME";
...
<p align="center"><span align="center" class="mgContentTitle"><a href="../reports/report.jsp">Reports</a>&nbsp;&nbsp;&raquo;&nbsp;&nbsp;User Login Failure Log</span></p>



<br>
<form name="userLoginFailLog" action="userLoginFail_log.jsp" method="post" >
	<table width="50%" border="1" align="center" class="sample">
		<tr class="tr_bgcolor">
                <td colspan="7" align="center">
                    <b>See User Failure Log</b>
...
<%
	...
	String strUserFailCount = "SELECT COUNT(*) FROM cms_login_failed_details";
	String strUserFailDetail ="SELECT ID, USER_NAME, DATE_FORMAT(LAST_LOGIN,'%d-%m-%Y %H:%i'), IP_ADDRESS FROM cms_login_failed_details ";
...
<%if(TotalPages > 1){%>
	<%if(Start > 1){%>
		<td><a href="userLoginFail_log.jsp?Start=<%=encrypt("1")%>&byDate=<%=strDate%>&userId=<%=strUserId%>"><font color="black"><b>First</b></font></a>&nbsp;&nbsp;</td>
		<td><a href="userLoginFail_log.jsp?Start=<%=encrypt(Integer.toString(Start-1))%>&byDate=<%=strDate%>&userId=<%=strUserId%>"><font color="black"><b>Previous</b></font></a>&nbsp;&nbsp;</td>
		<%}if(TotalPages > 1 && Start < TotalPages){%>
		<td>&nbsp;&nbsp;<a href="userLoginFail_log.jsp?Start=<%=encrypt(Integer.toString(Start+1))%>&byDate=<%=strDate%>&userId=<%=strUserId%>"><font color="black"><b>Next</font></b></a></td>
		<td>&nbsp;&nbsp;<a href="userLoginFail_log.jsp?Start=<%=encrypt(Integer.toString(TotalPages))%>&byDate=<%=strDate%>&userId=<%=strUserId%>"><font color="black"><b>Last</font></b></a></td>
		<%}%>
<%}%>
</tr>
</table>
