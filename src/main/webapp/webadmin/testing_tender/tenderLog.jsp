...
strTenderNo =   request.getParameter("tenderNo").replace("`","/").replace("|","&").trim();
...
<%
	...
	if( request.getParameter("tenderNo").length()==0 ){
	 ...
	 strTenderCount = "SELECT COUNT(*) FROM CMS_TENDER_DOWNLOAD_LOG as TDL,CMS_TENDER_MASTER_NEW as TM,CMS_VENDOR_LOGIN as VL WHERE TM.ID = TDL.TENDER_ID AND VL.VENDOR_ID = TDL.VENDOR_ID AND TENDER_ID IN (SELECT ID FROM CMS_TENDER_MASTER_NEW WHERE TM.ID LIKE '%"+strten_id+"%'AND TM.CREATED_ON<=TDL.DOWNLOAD_DATE)";
	 strTenderDetails ="SELECT TM.TENDER_NO,DATE_FORMAT(TDL.DOWNLOAD_DATE,'%d-%m-%Y %H:%i:%s'),VL.VENDOR_NAME,TDL.IP_ADDRESS,TDL.VENDOR_ID FROM CMS_TENDER_DOWNLOAD_LOG as TDL,CMS_TENDER_MASTER_NEW as TM,CMS_VENDOR_LOGIN as VL WHERE TM.ID = TDL.TENDER_ID AND VL.VENDOR_ID = TDL.VENDOR_ID AND TENDER_ID IN (SELECT ID FROM CMS_TENDER_MASTER_NEW WHERE TM.ID LIKE '%"+strten_id+"%' AND TM.CREATED_ON<=TDL.DOWNLOAD_DATE)";
	}
	else{
	
	strTenderCount = "SELECT COUNT(*) FROM CMS_TENDER_DOWNLOAD_LOG as TDL,CMS_TENDER_MASTER_NEW as TM,CMS_VENDOR_LOGIN as VL WHERE TM.ID = TDL.TENDER_ID AND VL.VENDOR_ID = TDL.VENDOR_ID AND TENDER_ID IN (SELECT ID FROM CMS_TENDER_MASTER_NEW WHERE TM.ID = '"+strten_id+"' AND TM.CREATED_ON<=TDL.DOWNLOAD_DATE)";
	strTenderDetails ="SELECT TM.TENDER_NO,DATE_FORMAT(TDL.DOWNLOAD_DATE,'%d-%m-%Y %H:%i:%s'),VL.VENDOR_NAME,TDL.IP_ADDRESS,TDL.VENDOR_ID FROM CMS_TENDER_DOWNLOAD_LOG as TDL,CMS_TENDER_MASTER_NEW as TM,CMS_VENDOR_LOGIN as VL WHERE TM.ID = TDL.TENDER_ID AND VL.VENDOR_ID = TDL.VENDOR_ID AND TENDER_ID IN (SELECT ID FROM CMS_TENDER_MASTER_NEW WHERE TM.ID ='"+strten_id+"' AND TM.CREATED_ON<=TDL.DOWNLOAD_DATE)";	
	}
