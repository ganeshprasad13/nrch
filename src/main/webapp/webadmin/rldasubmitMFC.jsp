...
try {
			...
			if(name.equals("DIV_ID")){
				divid=value.replaceAll("'","''");
				out.print("c");
			}			
			if(name.equals("STATION_ID")){
				stationid=value.replaceAll("'","''");
			}
			...
			if(name.equals("remark")){
				remark=value.replaceAll("'","''");
					...
					String newFileName = this.getServletContext().getRealPath("/uploads/MFC_Sites/") + 
...
strQuery="INSERT INTO  mfcstatus1 (station,railway,division,state,area,status,fileupload) VALUES(?,?,?,?,?,?,?)";
ps=cn.prepareStatement(strQuery);
 ps.setString(1,stationid); 
 ps.setString(2,zoneid);
 ps.setString(3,divid);
 ps.setString(4,stacat);
 ps.setString(5,area);
 ...
 ps.execute();
 out.print("success");
response.sendRedirect("rldaMFC.jsp?strMSG=Your Details have been Successfully Added.");
